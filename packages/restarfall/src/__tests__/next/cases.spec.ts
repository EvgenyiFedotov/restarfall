...
test("example for usage", () => {
  ...
  shapeApi.listenEvent(shape, $count.changed, console.log);
  shapeApi.attach(shape, counter());
  shapeApi.callEvent(shape, inc);
  shapeApi.callEvent(shape, dec);
...
test("component with args", () => {
  ...
  shapeApi.attach(shape, counter(10, -1));
...
test("component with children", () => {
  ...
  shapeApi.attach(shape, counter(true));
  shapeApi.attach(shape, counter(false));
...
test("update only chidlren", () => {
  ...
  shapeApi.attach(shape, counter());
  shapeApi.callEvent(shape, trigger);
  shapeApi.callEvent(shape, trigger);
...
test("strict order call children", () => {
  ...
  shapeApi.attach(shape, counter());
  shapeApi.callEvent(shape, trigger);
  shapeApi.callEvent(shape, reload);
